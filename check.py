...
__UNICODE_START_LIST = [start for start, _, _ in UNICODE_BLOCK_TABLE]


def main():
    if len(sys.argv) <= 1:
        print("Please input the font path")
        return
    font_file = Path(sys.argv[1])
    if not font_file.exists():
        print("Font does not exist")
        return
    coverage = get_unicode_coverage(font_file)
    print_coverage(coverage)


def get_unicode_coverage(font_file):
    font = TTFont(font_file)
    cmap = font["cmap"]
    unicode_maps = cmap.getBestCmap()
    unicode_blocks = {}
    for code_point in unicode_maps.keys():
        block_name = unicode_block_name(code_point)
        unicode_blocks[block_name] = unicode_blocks.get(block_name, []) + [
            chr(code_point)
        ]
...
def unicode_block_name(code_point):
    index = bisect_right(__UNICODE_START_LIST, code_point) - 1
    return UNICODE_BLOCK_TABLE[index][2]
